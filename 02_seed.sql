/* ================================================================
   SportPlusDB — Seed Data  (optimized)
...
/* ================================================================
   [0] XÓA DỮ LIỆU CŨ
   Thứ tự đúng FK: bảng con trước, bảng cha sau.
================================================================ */
...
/* ================================================================
   [1] LOOKUP TABLES
   ID phải khớp với comment StatusId trong schema và INSERT data bên dưới.
================================================================ */
...
-- StatusId: 1=Chờ TT   2=Đã xác nhận   3=Đã hủy   4=Đã hoàn thành   5=Chờ đặt cọc
INSERT INTO BookingStatuses(Name) VALUES
    (N'Chờ thanh toán'), (N'Đã xác nhận'), (N'Đã hủy'),
    (N'Đã hoàn thành'),  (N'Chờ đặt cọc');

-- StatusId: 1=Chưa TT   2=Đã TT   3=Thất bại   4=Đã hoàn tiền
...
/* ================================================================
   [2] SYSTEM CONFIG
   Các tham số vận hành — chỉnh qua sp_UpdateSystemConfig, không sửa thẳng.
================================================================ */
...
INSERT INTO Users(UserId, Email, Phone, PasswordHash, FullName, RoleId, StatusId, CreatedAt, UpdatedAt, IsDeleted) VALUES
...
('16:00','17:00',0),
('17:00','18:00',1), ('18:00','19:00',1), ('19:00','20:00',1),
('20:00','21:00',1), ('21:00','22:00',1);
GO
...
/* ================================================================
   ...
   T2–T6: theo IsPeakHour trong TimeSlots.
================================================================ */
...
/* ================================================================
   ...
   StockQty mô phỏng thực tế: một số sản phẩm sắp hết hàng (≤ MinQty).
================================================================ */
...
INSERT INTO Promotions(Code, Name, TypeId, DiscountValue, MaxDiscount, MinOrderAmount,
                       UsageLimit, UsageCount, StartDate, EndDate, IsActive, CreatedBy) VALUES
...
/* ================================================================
   ...
   BookingDetails không insert ở đây vì FieldSlots được sinh động theo ngày.
   SubTotal/TotalAmount đặt thủ công phản ánh dữ liệu thực tế hợp lý.
   Toàn bộ BookingId được RESEED về 0 → sẽ bắt đầu từ 1.
================================================================ */
...
/* ================================================================
   [14] PAYMENTS
   Dùng giá trị hardcode (1–13) an toàn vì RESEED=0 đảm bảo
   Bookings bắt đầu từ 1 khi chạy toàn bộ file.
================================================================ */
...
(1,  300000, 2, 1, NULL,                  N'Tiền mặt tại quầy',              DATEADD(DAY,-30,SYSDATETIME()), DATEADD(DAY,-30,SYSDATETIME())),
(2,  500000, 2, 2, N'MOMO202603250001',   N'MoMo transfer',                  DATEADD(DAY,-25,SYSDATETIME()), DATEADD(DAY,-25,SYSDATETIME())),
(3,  360000, 2, 2, N'MOMO202604010001',   N'MoMo transfer',                  DATEADD(DAY,-20,SYSDATETIME()), DATEADD(DAY,-20,SYSDATETIME())),
(4,  180000, 2, 2, N'MOMO202604050001',   N'MoMo transfer',                  DATEADD(DAY,-15,SYSDATETIME()), DATEADD(DAY,-15,SYSDATETIME())),
(5, 1000000, 2, 2, N'MOMO202604100001',   N'MoMo transfer',                  DATEADD(DAY,-12,SYSDATETIME()), DATEADD(DAY,-12,SYSDATETIME())),
(6,  480000, 2, 1, NULL,                  N'Tiền mặt',                       DATEADD(DAY,-10,SYSDATETIME()), DATEADD(DAY,-10,SYSDATETIME())),

-- Booking 7: hủy sớm → hoàn tiền 100%
(7,  200000, 2, 1, NULL,                  N'Thanh toán tiền mặt',            DATEADD(DAY,-8,SYSDATETIME()),  DATEADD(DAY,-8,SYSDATETIME())),
(7,  200000, 4, 1, NULL,                  N'Hoàn tiền - hủy đủ điều kiện',   DATEADD(DAY,-7,SYSDATETIME()),  DATEADD(DAY,-7,SYSDATETIME())),

-- Booking 8: hủy muộn → mất cọc 60k, hoàn 240k
(8,  300000, 2, 2, N'MOMO202604200001',   N'Thanh toán MOMO',                DATEADD(DAY,-6,SYSDATETIME()),  DATEADD(DAY,-6,SYSDATETIME())),
(8,  240000, 4, 2, N'REFUND20260421001',  N'Hoàn tiền (trừ cọc 60k)',        DATEADD(DAY,-5,SYSDATETIME()),  DATEADD(DAY,-5,SYSDATETIME())),

-- Booking 10–12: đã xác nhận, thanh toán đủ
(10, 300000, 2, 2, N'MOMO202605010001',   N'MoMo app',                       DATEADD(DAY,-2,SYSDATETIME()),  DATEADD(DAY,-2,SYSDATETIME())),
(11, 450000, 2, 2, N'MOMO202605020001',   N'MoMo',                           DATEADD(DAY,-1,SYSDATETIME()),  DATEADD(DAY,-1,SYSDATETIME())),
(12, 380000, 2, 3, N'VNPAY202605030001',  N'VNPay',                          DATEADD(HOUR,-5,SYSDATETIME()), DATEADD(HOUR,-5,SYSDATETIME())),

-- Booking 13: đặt cọc 220k (phần còn lại thanh toán khi đến sân)
(13, 220000, 2, 2, N'MOMO202605030002',   N'Thanh toán đặt cọc',             DATEADD(HOUR,-2,SYSDATETIME()), DATEADD(HOUR,-2,SYSDATETIME()));
GO
...
INSERT INTO Deposits(BookingId,RequiredAmount,PaidAmount,StatusId,DeadlineAt,PaidAt,ForfeitedAt,CreatedAt,UpdatedAt) VALUES
(8, 60000, 60000, 4,
...
INSERT INTO Deposits(BookingId,RequiredAmount,PaidAmount,StatusId,DeadlineAt,PaidAt,CreatedAt,UpdatedAt) VALUES
(13, 220000, 220000, 2,
...
INSERT INTO Deposits(BookingId,RequiredAmount,PaidAmount,StatusId,DeadlineAt,CreatedAt,UpdatedAt) VALUES
(14, 120000, 0, 1,
...
(1,  1, 1,  30000), (1,  3, 1, 120000),
(2,  2, 1, 150000), (2,  5, 1, 200000),
(3,  1, 2,  30000), (3,  3, 2, 120000),
(5,  2, 2, 150000), (5,  5, 1, 200000), (5,  3, 2, 120000),
(6,  1, 1,  30000),
(10, 1, 1,  30000),
(11, 2, 1, 150000), (11, 5, 1, 200000),
(13, 2, 2, 150000), (13, 5, 1, 200000), (13, 3, 3, 120000);
GO


/* ================================================================
   [17] REVIEWS — 1 review mỗi booking đã hoàn thành
================================================================ */

INSERT INTO Reviews(BookingId,UserId,FieldId,Rating,Comment,IsVisible,CreatedAt,UpdatedAt) VALUES
(1, 6,  1, 5, N'Sân A1 rất đẹp, cỏ mới, đèn sáng tốt. Nhân viên hỗ trợ nhiệt tình. Chắc chắn quay lại!',
 1, DATEADD(DAY,-28,SYSDATETIME()), DATEADD(DAY,-28,SYSDATETIME())),

(2, 7,  4, 4, N'Sân B1 rộng, mặt sân tốt. Tiếc là bãi xe hơi chật. Tổng thể hài lòng.',
 1, DATEADD(DAY,-24,SYSDATETIME()), DATEADD(DAY,-24,SYSDATETIME())),

(3, 8,  2, 5, N'Sân A2 ngoài trời nhưng mặt cỏ rất mượt. Giá hợp lý, đặt online tiện lợi. 5 sao!',
 1, DATEADD(DAY,-19,SYSDATETIME()), DATEADD(DAY,-19,SYSDATETIME())),

(4, 9,  3, 3, N'Sân ổn nhưng đèn một góc hơi mờ vào tối. Mong đội kỹ thuật kiểm tra lại.',
 1, DATEADD(DAY,-14,SYSDATETIME()), DATEADD(DAY,-14,SYSDATETIME())),

(5, 10, 5, 5, N'Sân B2 rất xịn, phòng thay đồ sạch sẽ, nhân viên chuyên nghiệp. Tổ chức giải đây nha!',
 1, DATEADD(DAY,-11,SYSDATETIME()), DATEADD(DAY,-11,SYSDATETIME())),

(6, 11, 1, 4, N'Sân sạch, dịch vụ thuê áo chất lượng tốt. Đặt sân qua app nhanh và dễ. Recommend!',
 1, DATEADD(DAY,-9,SYSDATETIME()),  DATEADD(DAY,-9,SYSDATETIME())),

-- Review ẩn do vi phạm nội dung (IsVisible=0)
(7, 12, 3, 1, N'[Review vi phạm - đã ẩn bởi admin]',
...
UPDATE Incidents SET HandledByUserId=1, HandledAt=DATEADD(DAY,-4,SYSDATETIME()),
    HandledNote=N'Đã siết lại chốt cửa, bôi trơn bản lề. Hoạt động bình thường.'
WHERE Title = N'Cổng vào sân A1 bị kẹt';

UPDATE Incidents SET HandledByUserId=1, HandledAt=DATEADD(DAY,-6,SYSDATETIME()),
...
UPDATE Incidents SET HandledByUserId=3, HandledAt=DATEADD(DAY,-9,SYSDATETIME()),
...
(15, N'Thanh toán thành công',
     N'Đã ghi nhận 450.000đ qua MoMo. Booking #11 đã xác nhận.',
...
SELECT [Table], [Count] FROM (
    SELECT N'Users (active)'        AS [Table], COUNT(*) AS [Count] FROM Users        WHERE IsDeleted = 0
    UNION ALL
    SELECT N'Fields (active)',                  COUNT(*)            FROM Fields       WHERE IsDeleted = 0 AND StatusId = 1
    UNION ALL
    SELECT N'TimeSlots',                        COUNT(*)            FROM TimeSlots
    UNION ALL
    SELECT N'FieldSlots (today+30d)',            COUNT(*)            FROM FieldSlots   WHERE SlotDate >= CAST(GETDATE() AS DATE)
    UNION ALL
    SELECT N'Bookings',                         COUNT(*)            FROM Bookings
    UNION ALL
    SELECT N'Payments',                         COUNT(*)            FROM Payments
    UNION ALL
    SELECT N'Deposits',                         COUNT(*)            FROM Deposits
    UNION ALL
    SELECT N'BookingServices',                  COUNT(*)            FROM BookingServices
    UNION ALL
    SELECT N'Reviews',                          COUNT(*)            FROM Reviews
    UNION ALL
    SELECT N'Incidents',                        COUNT(*)            FROM Incidents
    UNION ALL
    SELECT N'Notifications',                    COUNT(*)            FROM Notifications
    UNION ALL
    SELECT N'Promotions (active)',               COUNT(*)            FROM Promotions
        WHERE IsActive = 1 AND CAST(GETDATE() AS DATE) BETWEEN StartDate AND EndDate
    UNION ALL
    SELECT N'SpecialDays',                      COUNT(*)            FROM SpecialDays
    UNION ALL
    SELECT N'Products (low stock)',              COUNT(*)            FROM Products     WHERE IsDeleted = 0 AND StockQty <= MinQty
    UNION ALL
